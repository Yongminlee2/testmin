...
export default function ResultScreen() {
  ...
  const bands = variant === null ? [] : getGradeBands(gradeTableId('dialect', variant));
  const result =
  ...
  const retry = () => {
    ...
    const nextSeed = hashSeed(`dialect:${variant}:${Date.now()}`);
    ...
    start('dialect', variant, nextSeed, next);
      ...
      >
        <View ref={cardRef} collapsable={false}>
          <Certificate
            ...
            illustration={gradeComic('dialect', result.grade, variant)}
            note={
        ...
        <ShareButton targetRef={cardRef} dialogTitle={`사투리고사 · ${regionTitle}`} />
