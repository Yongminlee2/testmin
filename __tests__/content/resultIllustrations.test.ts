...
describe('결과 전용 일러스트', () => {
  ...
  test('모든 테스트×결과 조합 140개가 서로 다른 assetId를 쓴다', () => {
    expect(comics).toHaveLength(140);
    expect(new Set(comics.map((comic) => comic.assetId)).size).toBe(140);
    expect(resultIllustrationAssetIds()).toHaveLength(140);
    expect(new Set(resultIllustrationAssetIds()).size).toBe(140);
  });

  test('실제 WebP 140개의 경로와 파일 내용이 모두 중복되지 않는다', () => {
